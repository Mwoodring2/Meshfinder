#!/usr/bin/env python
import argparse, os, sqlite3, sys, traceback
from concurrent.futures import ThreadPoolExecutor, as_completed

# trimesh is already in your venv
import trimesh
import numpy as np

def compute_metrics(path: str):
    """
    Load a mesh with trimesh and return (tri_count, bbox_x, bbox_y, bbox_z).
    Returns None on failure.
    """
    try:
        m = trimesh.load(path, force='mesh')
        # Some files can load as scenes; try to combine
...
def worker(row):
    """row: (id, path) -> (id, tri_count, bx, by, bz) or (id, None, ...) on failure"""
    rid, path = row
...
def main():
    ...
    ap.add_argument("--where", default="tri_count = 0", help="SQL WHERE clause to select rows (default: tri_count = 0)")
    ...
    need_cols = {"id", "path", "tri_count", "bbox_x", "bbox_y", "bbox_z"}
